/* UMD build exporting the Docsify slides plugin function. */
(function (root, factory) {
  ...
  function initMermaidIn(root) {
    try {
      ...
      if (blocks && blocks.length) { window.mermaid.init(undefined, blocks); }
    } catch (e) {}
  }
  function renderMermaidInSlide(slideEl) {
    try {
      ...
      if (mermaidBlocks && mermaidBlocks.length) { window.mermaid.init(undefined, mermaidBlocks); }
    } catch (e) {}
  }
    ...
    containers.forEach(function(el){
      ...
      function update() {
        ...
        if (viewport) {
          ...
          try { if (window.mermaid) { var current = slides[idx]; renderMermaidInSlide(current); } } catch (e) {}
        }
        prevIdx = idx;
      }
  ...
  function plugin(hook, vm) {
    ...
    hook.beforeEach(function(){ setIdCounter.count = 0; });
    hook.afterEach(function(html, next){ next(replaceSlideBlocks(html, setIdCounter, defaults)); });
    hook.doneEach(function(){ wireSlides(document, defaults); setTimeout(function(){ initMermaidIn(document); }, 0); });
  }
  // Auto-install if Docsify global is present
